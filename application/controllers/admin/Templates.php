    ...
    public function create(){
        if($this->input->post()){
            ...
            $data = array(
                ...
                'type' => $this->input->post('type_configuration'),
                ...
                if($update){
                    ...
                    foreach ($result_common as $k => $val) {
                        ...
                        foreach ($arr_rcs as $key => $value) {
                            if(count($detail_rcs[$key]['choice']['vi']) > $value['count']){
                        ...
                        $table_lang_id = explode('|||', $val[$this->data['controller_use'].'_id']);
                        $table_lang_data = explode('|||', $val[$this->data['controller_use'].'_data_lang']);
                        foreach ($this->page_languages as $ks => $vals) {
                            $table_lang_data[$ks] = array_diff_key(json_decode($table_lang_data[$ks],true),array_flip($remove));
                            $table_lang_data[$ks] = empty($table_lang_data[$ks]) ? '{}' : json_encode($table_lang_data[$ks],true);
                        }
                        foreach ($rename_field as $key => $value) {
                            $update_data = str_replace('"'.$key.'":', '"'.$value.'":', $update_data);
                            foreach ($this->page_languages as $ks => $vals) {
                                $table_lang_data[$ks] = str_replace('"'.$key.'":', '"'.$value.'":', $table_lang_data[$ks]);
                            }
                        }
                        foreach ($add_field as $key => $value) {
                            $check_empty = ($update_data == '[]' || $update_data == '{}') ? '' : ',';
                            $check_empty_lang = ($table_lang_data[0] == '[]' || $table_lang_data[0] == '{}') ? '' : ',';
                            if($result[$value]['check_language'] == 'true'){
                                ...
                                foreach ($this->page_languages as $ks => $vals) {
                                    if($result[$value]['type'] == 'file' && isset($result[$value]['check_multiple'])){
                                        $table_lang_data[$ks] = substr_replace($table_lang_data[$ks],$check_empty_lang.'"'.$value.'":[]',-1,0);
                                    }else{
                                        $table_lang_data[$ks] = substr_replace($table_lang_data[$ks],$check_empty_lang.'"'.$value.'":""',-1,0);
                                    }
                                }
                        ...
                        $update_field[] =  array('id' => $val['id'], 'data' => $update_data);
                        foreach ($this->page_languages as $ks => $vals) {
                            $update_data_lang[$vals][] = array('id' => $table_lang_id[$ks], 'data_lang' => $table_lang_data[$ks]);
                        }
                    }
                    if($this->data['controller_use'] == 'product'){
                        $this->product_model->common_update_multiple($update_field);
                        foreach ($this->page_languages as $ks => $vals) {
                            $this->product_model->common_update_multiple_lang($update_data_lang[$vals]);
                        }
                    }else{

                        $this->post_model->common_update_multiple($update_field);
                        foreach ($this->page_languages as $ks => $vals) {
                            $this->post_model->common_update_multiple_lang($update_data_lang[$vals]);
                        }
                    }
                    $reponse = array(
                        'csrf_hash' => $this->security->get_csrf_hash(),
